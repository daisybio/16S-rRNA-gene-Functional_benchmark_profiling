...
map_data<-read.table("CRC/CRC_METADATA.tsv",sep="\t",header=TRUE,check.names = FALSE)
class(map_data$Sample_ID)<-"character"
#map_data<-dplyr::filter(map_data,DIABETES=="Healthy" | DIABETES=="Diabetes")
#map_data<-dplyr::filter(map_data,DIABETES=="Healthy" | DIABETES=="Diabetes")
KO_WGS<-read.table("CRC/CRC_MGS_KO.tsv",sep="\t",header = TRUE,check.names = FALSE,row.names=1)
...
file_list <- list.files(c("CRC/"),pattern = "_REL_KO.tsv",full.names = TRUE)
print(file_list)
...
process_element <- function(j) {
  ...
  CRC_p1 <- merge(Wilcox_p1_1, Wilcox_wgs_1, all = TRUE)
  CRC_p1_adj <- CRC_p1[, c(1, 2, 4)]
  colnames(CRC_p1_adj) <- c("KO", "prediction", "metagenom")
  CRC_p1_adj <- CRC_p1_adj[, c(1, 3, 2)]
  CRC_p1_p <- CRC_p1[, c(1, 3, 5)]
  colnames(CRC_p1_p) <- c("KO", "prediction", "metagenom")
  CRC_p1_p <- CRC_p1_p[, c(1, 3, 2)]
...
write.table(result_df,file="differential_abundance_accuracy_matrix_CRC.tsv",sep="\t",quote=FALSE,row.names = FALSE)